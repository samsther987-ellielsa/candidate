...
import { getGalleryIcon } from "@/lib/gallery-icons";
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4">
        {items.map((item, i) => {
          const Icon = getGalleryIcon(item.icon);
          return (
            <button
              key={item.id}
            ...
            >
              ...
              {/* 아이콘 (이미지 없을 때만) */}
              {!item.imagePath && (
                <div className="absolute top-4 left-4 opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all duration-300">
      ...
      {/* 라이트박스 */}
      {selected !== null && (() => {
        const SelectedIcon = getGalleryIcon(items[selected].icon);
        return (
          <div
